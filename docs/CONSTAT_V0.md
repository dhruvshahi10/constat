# CONSTAT_V0.md — Claude Code Build Directive
...
Before writing any code: read `README.md`, `constat/*.py`, `tests/test_gates.py`, one full `runs/<stamp>/` output including `audit_log.jsonl`. Produce a ≤20-line summary of the gate architecture and the answer contract. **No edits in this session.**
...
- Add Langfuse tracing behind env flag `CONSTAT_LANGFUSE=1`. **Gate:** all 11 tests + new fixtures green with live drafter.
