...
DATA = Path(os.environ.get("PRAMANA_DATA", "./data-hosted")).resolve()
DB_PATH = DATA / "pramana.db"
...
# rotate. (render.yaml still declares PRAMANA_SECRET; it is unused and can be
# dropped there whenever that file is next touched.)

PORT = int(os.environ.get("PORT", "8790"))
HOST = os.environ.get("PRAMANA_HOST", "127.0.0.1")
...
SIGNUPS_PER_DAY_GLOBAL = int(os.environ.get("PRAMANA_SIGNUPS_PER_DAY", "200"))
...
MAX_CONNECTIONS = int(os.environ.get("PRAMANA_MAX_CONNECTIONS", "64"))

# A run holding the single worker forever starves every other tenant.
RUN_BUDGET_S = int(os.environ.get("PRAMANA_RUN_BUDGET_S", "420"))
# Grace we give an over-budget run to notice it lost and stop, before the
# worker starts the next job alongside it.
ORPHAN_GRACE_S = int(os.environ.get("PRAMANA_ORPHAN_GRACE_S", "30"))

HOSTED_DRAFTER = os.environ.get("PRAMANA_DRAFTER", "gemini")
