# Northwind Health — security questionnaire response
...
**Your trust page** — A publishable page answering the 38 questions buyers ask most often. Only the 9 that are fully evidence-backed are answered; the rest are shown as open items a buyer must request. Host it, or send it as a file. The same answers as data are in trust_page/deflection.json.

### `evidence_gaps.md`
**Your work-list — the evidence gaps** — The 15 questions that could not be answered, grouped by reason, each naming the document to produce, renew or reconcile and who it was routed to. Close these and the same questions answer themselves next time.
...
- **9 answered with citations** — answers released with the document, version and location behind them.
- **15 refused** — no approved, in-force document supported an answer, so none was written.
- **15 open items** — the refusals, restated as documents somebody has to produce. This is `evidence_gaps.md`.
