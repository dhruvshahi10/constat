"""Constat hosted server. stdlib only: ThreadingHTTPServer + route table.
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _dispatch(self, method: str) -> None:
        ...
        for m, pat, name, needs_auth in ROUTES:
            ...
            try:
                ...
                sys.stderr.write(f"[constat] 500 on {method} {path}:\n"
    ...
    def landing(self) -> None:
        ...
        if site.is_file():
            ...
            self._html("<title>Constat</title><p>Constat hosted. Landing ships in Wave 2. "
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Constat hosted server")
    ...
    sys.stderr.write(f"[constat] serving on http://{args.host}:{args.port}\n")
    srv.serve_forever()
